...
export default function AIAnalysisView() {
  ...
  const analyzeFloodRisk = useCallback(async () => {
    ...
    try {
      const resp = await apiClient.post("/ai/analyze-image/", {
        image_url: imageUrl,
      });

      // Backend returns { analysis: {...}, recommendations: [...] }
      const payload = resp.data?.analysis ? resp.data.analysis : resp.data;
      setResult(payload as AnalysisResult);
    } catch (err: any) {
      console.error("[FloodVision] Analyze error:", err);
      const cannotConnect = !err?.response;
      setError(
        cannotConnect
  ...
  const riskStyle = result
    ? (RISK_COLORS[result.risk] ?? RISK_COLORS.UNKNOWN)
    : null;
    ...
    <div className="min-h-full w-full bg-[#020617] text-white p-4 md:p-6 pb-12">
        ...
        <div className="bg-white/5 border border-cyan-500/20 rounded-2xl overflow-hidden backdrop-blur-xl">
          <div className="px-4 py-3 border-b border-white/8 flex items-center gap-2">
            ...
            <span className="ml-auto text-xs text-slate-500 font-mono">
              Click map or drag pin to change location
            </span>
          </div>
            ...
            <>
              ...
              >
                <path
                  ...
                  d="M9.75 3.104A8.25 8.25 0 1 0 20.896 14.25M15 3v6h6"
                />
              </svg>
      ...
      {error && (
        <div className="mb-6 flex items-start gap-3 bg-red-500/10 border border-red-500/30 rounded-xl p-4">
          ...
          <div>
            <p className="text-red-300 font-semibold text-sm mb-0.5">
              Analysis Failed
            </p>
        ...
        >
          ...
          <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
            ...
            <div className="text-right">
              <div
                className={`text-6xl font-black leading-none ${riskStyle!.text}`}
              >
                {result.score}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {result.reasons.map((reason, i) => (
                    ...
                    >
                      <span
                        className={`mt-0.5 text-base shrink-0 ${riskStyle!.text}`}
                      >
                        {i === 0 ? "◉" : "◈"}
                      </span>
                      {reason}
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {["YOLOv8n", "OpenCV 4.x", "NumPy", "Django REST"].map(
                  (tag) => (
                    <span
                      key={tag}
            ...
            <div className="flex flex-col gap-3">
              ...
              {result.processed_image ? (
                <div
                  className={`rounded-xl overflow-hidden border ${riskStyle!.border} relative`}
                >
                  <img
